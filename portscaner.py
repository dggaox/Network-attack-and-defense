...
class PortScaner(object):
    ...
    def banber(self):
        logo = '''
            网络攻防实验之端口扫描
            欢迎使用端口扫描器 V1.0
            姓名：高翔 学号：1708324057 班级：2017网络工程
        '''
        return logo
    ...
    def help(self):
        help = """
[使用方法]
    python portscaner.py -i ip -p [port_scope|top_num] [-t thread_num]
    python portscaner.py -u url -p [port_scope|top_num] [-t thread_num]\n\n
[示例]
    在Linux下输入命令：
    python3 portscaner.py -i 127.0.0.1 -p 1000 -t 100
    python3 portscaner.py -u https://www.baidu.com -p 1~65535
    其中-i 表示对特定IP地址服务器扫描 -u 表示对特定网址服务器扫描
    -p 表示端口号 -t表示多线程进程数量

[参数取值范围]
    ip              0.0.0.0-255.255.255.255
    top_num         [50|100|1000(default)]【单进程时使用】
    port_scope      1-65535
...
def main():
    ...
    print("[总共用时] %3ss"%(end_time-start_time,))
